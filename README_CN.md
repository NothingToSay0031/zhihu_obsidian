...
- 递归批量发布文件夹中的所有 Markdown 笔记，并显示进度和汇总。
- 发布时自动将 Mermaid 代码块转为图片并上传知乎图床。
...
在写完文章准备发表的时候，运行命令 `Zhihu: Publish current article` 即可。  
如果当前笔记没有 frontmatter，或者缺少 `zhihu-topics`，插件会在发布前自动补齐：

- `zhihu-title`：默认使用文件名
- `zhihu-topics`：默认使用标题

然后再将 markdown 转换为知乎 HTML 并发布。在真正看到知乎上的文章之前，你可能需要等上几秒（或几分钟）。

### 批量发布文件夹

运行命令 `Zhihu: 发布文件夹内全部笔记到知乎`，选择目标文件夹后即可批量发布。

- 会递归发布该文件夹下所有 `.md` 文件。
- 可选择“跳过已发布笔记”或“覆盖已发布笔记”（根据 `zhihu-link` 判断）。
- 每篇发布都有进度提示，结束后显示成功/跳过/失败汇总。
- 每篇之间的间隔可在设置中调整（`批量发布间隔（秒）`，默认 `5`）。
...
发布时会自动检测 ` ```mermaid ... ``` ` 代码块，将其渲染为图片并上传到知乎，再替换为图片标签后提交。
该转换仅发生在发布流程中，不会修改你本地的 markdown 文件内容。
